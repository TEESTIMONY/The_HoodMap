...
/**
 * Kinetic Matrix — a spring-mass lattice that reacts to the pointer, with
 * synaptic pulses and click shockwaves. Recoloured for HoodMap: near-black
 * ground, quiet-ink lattice, lime accent on everything under tension.
 * (Adapted from a 21st.dev component.)
 */
...
// HoodMap palette (rgb triples for rgba() interpolation)
...
export function KineticMatrix({
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      if (!reduceMotion && Math.random() < 0.3 && nodes.length > 0 && pulses.length < 40) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        if (isNear || n.tension > 0.1) {
          ctx.fillStyle = `rgba(${LIME}, ${Math.min(1, 0.18 + n.tension * 0.5)})`;
          ctx.beginPath();
          ctx.arc(n.x, n.y, currentRadius * 2.4, 0, Math.PI * 2);
          ctx.fill();
        }

        // Ambient life: hub nodes carry a faint lime breath even at rest.
        const isHub = (n.col + n.row) % 6 === 0;
        const restColor = isHub
          ? `rgba(${LIME}, ${0.18 + Math.sin(n.pulsePhase) * 0.08})`
          : `rgba(${INK_FAINT}, 0.5)`;
        ctx.fillStyle = isNear || n.tension > 0.1 ? `rgb(${LIME})` : restColor;
        ctx.beginPath();
        ...
        if (dist < 90) {
          ...
          ctx.strokeStyle = `rgba(${LIME}, ${ringAlpha})`;
          ctx.lineWidth = 1;
          ctx.beginPath();
  ...
  const drawLatticeLink = (
    ...
    const tensioned = n1.tension > 0.05 || n2.tension > 0.05 || stretch > 0.1;

    if (tensioned) {
      const glow = Math.max(n1.tension, n2.tension, stretch * 2);
      ctx.strokeStyle = `rgba(${LIME}, ${Math.min(1, 0.2 + glow * 0.7)})`;
      ctx.lineWidth = 0.8 + glow * 1.4;
    } else {
      ctx.strokeStyle = `rgba(${INK_FAINT}, 0.14)`;
      ctx.lineWidth = 0.65;
    }
  ...
  return (
    <div
      ...
      className={cn(
        "group relative flex h-full w-full select-none flex-col justify-between overflow-hidden bg-canvas",
        className
      )}
    >
      ...
      {(showControls || showTitle) && (
        <div className="relative z-20 flex h-full w-full flex-col justify-between p-6 md:p-10">
          {showControls ? (
            <header className="flex w-full items-center justify-between font-mono text-[11px] text-ink-muted">
              <div className="flex items-center gap-2">
                <button
                  onClick={triggerCentralImpulse}
                  className="flex items-center gap-1.5 rounded-lg border border-line-strong bg-surface-2/70 px-2.5 py-1.5 backdrop-blur-md transition-colors hover:border-lime/40 hover:text-lime"
          ...
          {showTitle ? (
            <main className="pointer-events-none flex flex-col items-center justify-center text-center">
              <h1 className="font-mono text-5xl font-black uppercase tracking-tighter text-ink sm:text-7xl md:text-9xl">
                {title}
              </h1>
            </main>
          ) : (
